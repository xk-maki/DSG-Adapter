        ...
        if downsample is not None:
            if num_prompts is None:
                ...
                self.downsample = downsample(
                    num_prompts, prompt_location, deep_prompt, input_resolution=input_resolution,
                    dim=dim, norm_layer=norm_layer
                )
        else:
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = self.norm(x)
            '''wxk新增'''
            # - transpose(1,2)：将(B, Ph*Pw, C)转回(B, C, Ph*Pw)
            # - view(-1, self.embed_dim, Wh, Ww)：重塑为(B, 嵌入维度, 高, 宽)，恢复成4维特征图
