...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, tuning_mode=tuning_mode)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, tuning_mode=None):
        super().__init__()
        self.dim = dim
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # build blocks
        # 如果 tuning_mode 是一个列表，则为每个 block 分配对应的模式；否则所有 block 使用相同的模式
        if isinstance(tuning_mode, list):
            block_tuning_modes = tuning_mode
        else:
            block_tuning_modes = [tuning_mode] * depth
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                tuning_mode=block_tuning_modes[i])
            for i in range(depth)])

        # patch merging layer
        if downsample is not None:
            self.downsample = downsample(dim=dim, norm_layer=norm_layer, tuning_mode=tuning_mode)
        else:
...
class SwinTransformer(nn.Module):
    ...
    Args:
        ...
        use_checkpoint (bool): Whether to use checkpointing to save memory. Default: False.
        tuning_mode (str, optional): Tuning mode, e.g., 'ssf'. Default: None.
    """
    ...
    def __init__(self,
        ...
        # 为每一层准备 tuning_mode 列表
        tuning_mode_list = []
        if tuning_mode == 'ssf':
            for i_layer in range(self.num_layers):
                tuning_mode_list.append([tuning_mode] * depths[i_layer])
        else:
            tuning_mode_list = [None] * self.num_layers
        ...
        # 如果是 SSF 微调，为最后的输出特征添加 scale 和 shift
        if self.tuning_mode == 'ssf':
            # 这里假设对所有输出层级都应用相同的最后一个变换，或者可以根据需要为每个 out_index 单独添加
            # 参考 swin_transformer.py 的实现，它是在所有层之后，avgpool 之前对 num_features 进行变换
            # 由于此版本是输出多尺度特征，我们可以在每个输出前应用，或者在最后统一应用。
            # 为了与原始 SSF 逻辑更接近，我们为每个输出通道的维度添加 scale/shift
            self.ssf_scales = nn.ParameterList()
            self.ssf_shifts = nn.ParameterList()
            for i in out_indices:
                scale, shift = init_ssf_scale_shift(num_features[i])
    ...
    def init_weights(self, pretrained=None):
        ...
        for name, param in self.named_parameters():
            # 判断参数名是否包含 SSF 特有的关键词
            # 我们添加的参数名包含 'ssf_scale' 或 'ssf_shift'
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                if self.tuning_mode == 'ssf':
                    ...
                    x_out = ssf_ada(x_out, self.ssf_scales[idx], self.ssf_shifts[idx])
